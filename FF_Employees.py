...
def employees():
    ...
    print(r.url)
    file = open('C:/FTPData/Hardy/XML Files/FFemployees.xml', 'w')
    file.write(r.text)
    file.close()
    tree = ET.parse('C:/FTPData/Hardy/XML Files/FFemployees.xml')

    root = tree.getroot()
    timestr = time.strftime("%m%d%Y-%H%M%S")
    data = open('C:/FTPData/Hardy/Family Ford/FFemployees'+timestr+'.csv', 'w')
